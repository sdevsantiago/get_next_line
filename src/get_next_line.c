/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   get_next_line.c                                    :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: sede-san <sede-san@student.42madrid.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/14 18:58:57 by sede-san          #+#    #+#             */
/*   Updated: 2026/08/07 00:45:06 by sede-san         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static char	*fill_buffer(int fd, char *buffer);
static char	*resize_buffer(char *buffer);

/* Reads the next line from the file pointed by FD. If no lines have been read
   previously, reads the first one. */
char	*get_next_line(
	int fd)
{
	char		*line;
	static char	*buffer;
	size_t		len;

	if (fd < 0 || BUFFER_SIZE < 1)
		return (NULL);
	if (!buffer)
		buffer = gnl_strdup("");
	len = (uintptr_t)gnl_strchr(buffer, EOL);
	if (len)
	{
		len -= (uintptr_t)buffer;
		line = gnl_substr(buffer, 0, len + 1);
		buffer = resize_buffer(buffer);
		return (line);
	}
	buffer = fill_buffer(fd, buffer);
	if (!buffer || !*buffer)
		return (free(buffer), buffer = NULL, NULL);
	len = (uintptr_t)gnl_strchr(buffer, EOL) - (uintptr_t)buffer;
	line = gnl_substr(buffer, 0, len + 1);
	buffer = resize_buffer(buffer);
	return (line);
}

/* Fills the buffer until it contains a full line. A complete line is
   understood to be one that contains an EOL. */
static char	*fill_buffer(
	int fd,
	char *buffer)
{
	char	*buf;
	ssize_t	len;

	buf = (char *)malloc((BUFFER_SIZE + 1) * sizeof(char));
	if (!buf)
		return (free(buffer), NULL);
	len = read(fd, buf, BUFFER_SIZE);
	if (len <= 0)
	{
		if (buffer)
			return (free(buf), buffer);
		return (free(buf), free(buffer), NULL);
	}
	buf[len] = '\0';
	if (!gnl_strchr(buf, EOL) && len == BUFFER_SIZE)
	{
		buffer = gnl_strjoin(buffer, buf);
		return (free(buf), fill_buffer(fd, buffer));
	}
	buffer = gnl_strjoin(buffer, buf);
	if (!buffer)
		return (free(buffer), NULL);
	return (free(buf), buffer);
}

/* Resizes the buffer removing the line contained. If emptied, the buffer is
   freed and its node is deleted from the list FILES. */
static char	*resize_buffer(
	char *buffer)
{
	char		*new_buffer;
	size_t		len;
	uintptr_t	i;

	i = (uintptr_t)gnl_strchr(buffer, EOL);
	if (i)
	{
		i -= (uintptr_t)buffer;
		len = gnl_strlen(&buffer[i + 1]);
	}
	else
		len = gnl_strlen(buffer);
	if ((i || buffer[i] == EOL) && buffer[i + 1])
		new_buffer = gnl_substr(buffer, i + 1, len + 1);
	else
		new_buffer = NULL;
	free(buffer);
	return (new_buffer);
}
